    ...
    public class Inventory_part : Update_pstgr_from_Ora<Inventory_part.Inventory_part_row>
    {
        ...
        public async Task<List<Inventory_part_row>> Get_Ora_list(string Task_name, CancellationToken cancellationToken) => await Check_length(await rw.Get_Ora("" +
               String.Format(@"SELECT 
                    ...
                    ifsapp.Inventory_Part_API.Get_Volume_Net(contract, PART_NO) Volume_Net,
                    ifsapp.inventory_part_unit_cost_api.Get_Inventory_Value_By_Config(contract, PART_NO,'*') Inventory_Value,
                    NOTE_ID 
                FROM (
                        SELECT 
                            part_no,
